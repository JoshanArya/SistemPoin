        ...
        <div class="col-md-8">
            <form action="" method="POST" class="d-flex justify-content-md-end gap-2">
                ...
                <div class="dropdown w-25 border rounded">
                    <button class="btn dropdown-toggle-filter dropdown-toggle w-100 text-start" type="button"
                        id="dropdown_kelas" data-bs-toggle="dropdown">
                        <?= !empty($kelas_filter) ? htmlspecialchars($kelas_filter) : 'Semua Kelas' ?>
                    </button>
                    <ul class="dropdown-menu kelas w-100 text-start">
                        <li>
                            <a class="dropdown-item" href="#"
                                onclick="setDropdown('kelas', 'dropdown_kelas', 'Semua Kelas', '')">
                                Semua Kelas
                            </a>
                        </li>
                        <?php while ($k = mysqli_fetch_assoc($query_kelas_dropdown)): ?>
                            <li>
                                <a class="dropdown-item" href="#"
                                    onclick="setDropdown('kelas', 'dropdown_kelas', this.innerText, this.innerText)">
                ...
                <div class="dropdown w-25 border rounded">
                    <input type="hidden" name="status_siswa" id="status"
                    ...
                    <ul class="dropdown-menu w-100 text-start">
                        <li><a class="dropdown-item" href="#"
                                onclick="setDropdown('status', 'dropdown_status', 'Semua Status', '')">Semua Status</a>
                        </li>
                        <li><a class="dropdown-item" href="#"
                ...
                <a href="add.php" class="btn btn-primary w-25 py-2"><i class="bi bi-person-fill-add me-1"></i>Tambah
                    Murid</a>
            </form>
        </div>
    </div>
                        ...
                        <?php foreach ($result as $siswa): ?>
                            <tr>
                                ...
                                <td class="text-center d-flex justify-content-center gap-1  px-0">
                                    <a class="btn-action d-inline-block btn-detail" title="Detail"
                                        href="details.php?nis=<?= $siswa['nis'] ?>"><i class="bi bi-eye-fill"></i></a>
                                    <a class="btn-action btn-edit" href="edit.php?nis=<?= $siswa['nis'] ?>" title="Edit"><i
                                            class="bi bi-pencil-fill"></i></a>
                                    <form action="/SistemPoin/process/siswa_process.php" method="post"
                                        onsubmit="return confirm('Ingin Menghapus data <?= $siswa['nama_siswa'] ?>?')">
                                        <!-- Kirim id dan action ke file proses -->
                                        <input type="hidden" name="nis" value="<?= $siswa['nis'] ?>">
                                        <input type="hidden" name="action" value="delete">
                                        <button class="btn-action btn-delete" title="Hapus" type="submit"><i
